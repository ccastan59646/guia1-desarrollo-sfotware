...
public class FeriaEmpresarial {
    ...
    // Puntos de Extensi�n
    // -----------------------------------------------------------------
    // para determinar el número de expositores.
    private int numeroExpositores(String nombreEmpresa) {
    ...
    public String metodo1() {
        // se realiza un bucle del objeto puestos
        for (Puesto p: puestos) {
            if (p.estaOcupado()) {
                // se condiciona el puesto en una zona específica.
                if (p.darZona().equals(Puesto.ZONA_OCCIDENTE)) {
                    return p.darNombreEmpresa() + 
                    numeroExpositores(p.darNombreEmpresa());
                }
            }
        }
        return "";
